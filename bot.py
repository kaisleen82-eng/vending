            ...
            "components": [
                ...
                {"type": 14, "spacing": 1, "divider": True},
                {
                    ...
                    "components": [
                        ...
                        {"type": 2, "label": "Get Link", "style": 2, "custom_id": "panel_getlink",
...
async def on_interaction(interaction):
    ...
    if custom_id == "panel_register":
    ...
    elif custom_id == "panel_getlink":
        embed = discord.Embed(
            title="Coming Soon",
            description="The script link is not available yet. Check back later.",
            color=DARK
        )
        embed.set_footer(text=f"{APP_NAME} | Key System")
        await interaction.response.send_message(embed=embed, ephemeral=True)
        ...
        if role:
            if role in member.roles:
                embed = discord.Embed(title="Already Verified", description="You already have the Verified role.", color=DARK)
            else:
                await member.add_roles(role)
                embed = discord.Embed(title="Role Assigned", description="You now have the **Verified** role.", color=DARK)
        else:
            embed = discord.Embed(title="Role Not Found", description="The **Verified** role does not exist.", color=discord.Color.red())
        embed.set_footer(text=f"{APP_NAME} | Key System")
        await interaction.response.send_message(embed=embed, ephemeral=True)
